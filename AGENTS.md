...
このアプリは 10 年以上の大切な写真（数万枚・数TB規模）を安全に管理するため、以下の防御的設計を採用しています。

#### 保存先ファイルが消えた場合の再取り込み

メタデータに「取り込み済み」と記録されていても、保存先に実際のファイルが存在しない場合は**再取り込み**します。これにより以下のケースに対応できます。

- ユーザーが誤って保存先のファイルを削除してしまった
- ストレージ障害でファイルが失われた
- 別の PC に移動したため保存先が変わった

**実装**: `_buildImportTargets` で `destFile.exists()` を毎回確認し、存在しなければ candidates に追加して再取り込みする。

#### 保存先ファイルの破損検出

メタデータに記録があり保存先にファイルが存在する場合でも、**軽量シグネチャ**（ファイルサイズ + 先頭/末尾のハッシュ）で同一性を検証します。シグネチャが一致しない場合は以下を行います。

- ファイルが破損している可能性があるとみなす
- 別名（サフィックス付き）で再取り込みする
- 警告（`ImportWarningType.DuplicateRenamed`）を記録し、ユーザーに通知する

これにより、突然のシャットダウンなどで JPEG が途中までしか書き込まれていないケースを検出できます。

**重要**: この設計のため、取り込み対象判定では**保存先ファイルの存在確認と検証を省略してはならない**。性能最適化で保存先確認をスキップすると、上記の安全機能が無効化されるため禁止。

#### コピー時のハッシュ検証

ファイルコピー後に、元ファイルとコピー先のハッシュを比較します。一致しない場合は最大 3 回リトライし、それでも失敗すれば中断します。これにより、転送エラーによる破損を防止します。

#### メタデータの安全な書き込み

`METADATA.JSON` は一時ファイルに書き込んでからリネームすることで、書き込み中のクラッシュによる破損を防止します。また、ロックファイルで排他制御を行い、複数プロセスからの同時書き込みを防ぎます。破損したメタデータファイルはタイムスタンプ付きでバックアップされます。

#### 容量チェックの二重確認

取り込み開始前に全体の必要容量をチェックし、さらに各ファイルコピー前にも残り必要容量と現在の空き容量を比較します。これにより、他のプロセスがディスクを消費した場合でも安全に中断できます。

#### コピー失敗時の残骸削除

ファイルコピーが途中で失敗した場合、不完全なコピー先ファイルを削除してからエラーを伝播します。これにより、壊れたファイルが保存先に残ることを防ぎます。
...
1. **依存関係バージョンの無断変更禁止** - 変更が必要な場合は理由を明示して承認を得る
2. **既存コメントの削除禁止** - 設計意図が含まれているため保持する
3. **取り込み元ファイルの削除処理の実装禁止** - データ安全性の原則に反する
