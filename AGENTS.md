...
このプロジェクトは「取りこぼしゼロ」「破壊ゼロ」を最優先とする。メディアファイルの削除や上書きなどの不可逆な処理は実装しない。コピー先で同名衝突が発生した場合は、` (1)` 形式のサフィックスで必ず両方を残す。

ログ出力は必ず英語で記述し、文末は原則ピリオドで締める。ただし `Error: ...` や `Exception: ...` のようにコロンで値を続ける形式は例外とする。ユーザー向けメッセージ、とくに Snackbar の文言は日本語で記述し、文末に必ず「。」を付ける。

設定項目、モデルのフィールド定義、関数引数の並びは UI 表示順に合わせて記述する。表示順とコード上の順序が一致していない場合は、順序の是正を優先する。

UI/UX の見た目に影響する変更（レイアウト、色、フォント、間隔、表示順の変更を含む）は事前承認が必須である。挙動の変更が UI 表示に影響する場合も、変更理由を明確にしてから進める。

SD カード側の取り込み済みメタデータは `PRIVATE/AIU/METADATA.JSON` に保存する。取り込み完了後にのみ記録し、未完了のファイルは記録しない。
